import { motion } from "framer-motion";
import { Euro, TrendingDown, Leaf, ArrowRight } from "lucide-react";
import DPEGauge from "./DPEGauge";

function StatCard({ icon: Icon, label, value, unit, color }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className={`rounded-xl border p-4 ${color}`}
    >
      <div className="flex items-center gap-2 mb-1">
        <Icon size={16} className="opacity-60" />
        <span className="text-xs font-medium opacity-70">{label}</span>
      </div>
      <p className="text-2xl font-bold">
        {typeof value === "number" ? value.toLocaleString("fr-FR") : value}
        <span className="text-sm font-normal ml-1 opacity-60">{unit}</span>
      </p>
    </motion.div>
  );
}

function ComparisonBar({ label, current, improved, unit, color }) {
  const maxVal = Math.max(current, improved, 1);
  return (
    <div className="space-y-1">
      <p className="text-xs font-medium text-gray-500">{label}</p>
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <div className="h-5 bg-gray-100 rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(current / maxVal) * 100}%` }}
              transition={{ duration: 0.8 }}
              className={`h-full rounded-full ${color}`}
            />
          </div>
          <p className="text-[10px] text-gray-400 mt-0.5">Actuel : {current} {unit}</p>
        </div>
        <ArrowRight size={14} className="text-gray-300 shrink-0" />
        <div className="flex-1">
          <div className="h-5 bg-gray-100 rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(improved / maxVal) * 100}%` }}
              transition={{ duration: 0.8, delay: 0.3 }}
              className="h-full rounded-full bg-eco-400"
            />
          </div>
          <p className="text-[10px] text-eco-600 mt-0.5">Après travaux : {improved} {unit}</p>
        </div>
      </div>
    </div>
  );
}

export default function Dashboard({ result }) {
  if (!result) return null;
  const { diagnostic, recommandations } = result;

  const bestConso = recommandations.length > 0
    ? Math.min(...recommandations.map((r) => r.nouvelle_conso_kwh_m2))
    : diagnostic.consommation_kwh_m2_an;

  const totalEconomie = recommandations.reduce((s, r) => s + r.economie_annuelle_euros, 0);

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-6"
    >
      {/* Stat cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard
          icon={TrendingDown}
          label="Consommation"
          value={diagnostic.consommation_kwh_m2_an}
          unit="kWh/m²/an"
          color="bg-orange-50 border-orange-200 text-orange-800"
        />
        <StatCard
          icon={Leaf}
          label="Émissions CO₂"
          value={diagnostic.emission_co2_kg_m2_an}
          unit="kg CO₂/m²/an"
          color="bg-blue-50 border-blue-200 text-blue-800"
        />
        <StatCard
          icon={Euro}
          label="Coût annuel"
          value={diagnostic.cout_annuel_euros}
          unit="€/an"
          color="bg-emerald-50 border-emerald-200 text-emerald-800"
        />
      </div>

      {/* DPE / GES Gauges */}
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-4 uppercase tracking-wide">
          Étiquettes énergie & climat
        </h3>
        <DPEGauge diagnostic={diagnostic} />
      </div>

      {/* Comparison */}
      {recommandations.length > 0 && (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 space-y-4">
          <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">
            Avant / Après travaux
          </h3>
          <ComparisonBar
            label="Consommation énergétique"
            current={diagnostic.consommation_kwh_m2_an}
            improved={bestConso}
            unit="kWh/m²/an"
            color="bg-orange-400"
          />
          <ComparisonBar
            label="Coût annuel"
            current={diagnostic.cout_annuel_euros}
            improved={Math.max(0, diagnostic.cout_annuel_euros - totalEconomie)}
            unit="€"
            color="bg-red-400"
          />
        </div>
      )}
    </motion.div>
  );
}
